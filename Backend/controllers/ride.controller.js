...
module.exports.createRide = async (req, res) => {
    try {
        ...
        // Get user details for the ride
        const user = await userModel.findById(ride.user);
        const rideWithUser = { ...ride, user };
...
module.exports.endRide = async (req, res) => {
    try {
        ...
        const { rideId, duration, distance } = req.body;
...
module.exports.getUserRides = async (req, res) => {
    try {
        const rides = await rideService.findRidesByUserId(req.user.id);
...
module.exports.getCaptainRides = async (req, res) => {
    try {
        const rides = await rideService.findRidesByCaptainId(req.captain.id);
        return res.status(200).json(rides);
    } catch (err) {
        return res.status(500).json({ message: err.message });
    }
